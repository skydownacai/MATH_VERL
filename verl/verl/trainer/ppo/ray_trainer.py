    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "wandb" in configured_loggers or "tracking" in configured_loggers:
                    advantage_histogram = compute_advantage_histogram(
                        batch=batch,
                        bins=self.config.trainer.get("advantage_histogram_bins", 20),
                    )
                    if advantage_histogram is not None:
                        import wandb

                        logger.log(
                            data={"critic/advantages/histogram": wandb.Histogram(np_histogram=advantage_histogram)},
