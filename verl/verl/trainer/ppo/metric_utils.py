...
def compute_advantage_histogram(batch: DataProto, bins: int = 50) -> tuple[np.ndarray, np.ndarray] | None:
    """Compute histogram for response-masked advantages.

    This helper is intended for visualization backends like wandb.

    Args:
        batch: A DataProto object containing advantages and response_mask.
        bins: Number of histogram bins.

    Returns:
        A tuple of (counts, bin_edges) for np_histogram format, or None when no valid token exists.
    """
    if bins <= 0:
        raise ValueError(f"bins must be greater than 0, got {bins}")

    response_mask = batch.batch["response_mask"].bool()
    advantages = batch.batch["advantages"]
    valid_advantages = torch.masked_select(advantages, response_mask)

    if valid_advantages.numel() == 0:
        return None

    valid_advantages_np = valid_advantages.detach().float().cpu().numpy()
